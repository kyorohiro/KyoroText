	...
	public void binaryKey(MyInputConnection conn, int keycode, boolean shift, boolean ctl, boolean alt) 
	{
		...
		if(mKeyMap.containKey(keycode, shift)){
			...
			if(KeyMap.KEYCODE_BACKSPSACE == keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_BACK));
			}
			else if(KeyMap.KEYCODE_ENTER == keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_ENTER));
			}
			else if(KeyMap.KEYCODE_DPAD_DOWN == keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_DPAD_DOWN));
			}
			else if(KeyMap.KEYCODE_DPAD_LEFT == keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_DPAD_LEFT));
			}
			else if(KeyMap.KEYCODE_DPAD_RIGHT == keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_DPAD_RIGHT));
			}
			else if(KeyMap.KEYCODE_DPAD_UP== keycode) {
				conn.addCommitText(new CommitText(KeyEvent.KEYCODE_DPAD_UP));
			}
		}
