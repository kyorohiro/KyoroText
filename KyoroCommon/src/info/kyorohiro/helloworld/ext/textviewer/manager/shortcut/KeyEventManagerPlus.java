...
//		android.util.Log.v("kiyo","onUpdate:"+mode);
		...
		else if(BufferManager.SHELL_BUFFER.equals(mode)) {
			getManager().updateCommnad(EMACS_SHORTCUT_SHELL);
		} else {
			getManager().updateCommnad(EMACS_SHORTCUT_VIEW);
		}
	}

	public static Command[] EMACS_SHORTCUT_SHELL = {
		...
		new Command(new CommandPart[]{new CommandPart(SimpleKeyEvent.KEYCODE_ENTER, false, false)}, new ShellTaskDone()),	
