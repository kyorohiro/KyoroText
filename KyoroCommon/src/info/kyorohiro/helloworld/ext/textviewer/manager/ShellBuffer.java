...
// now creating 
//
public class ShellBuffer extends TextViewer {

	public static final String SHELL_BUFFER = CursorableLineView.MODE_EDIT+" shell";
	private MiniBufferJob mMiniBufferJob = null;

	public static ShellBuffer newShellBuffer(BufferManager manager, int textSize, int width, int mergine, boolean message) {
		ShellBuffer ret = null;
		File file = new File(BufferManager.getManager().getApplication().getApplicationDirectory(),"shellbuffer");
		...
		try {
			...
			ret = new ShellBuffer(manager, buffer, textSize, width, mergine);
			ret.getLineView().isTail(true);
	...
	private ShellBuffer(BufferManager manager, TextViewerBuffer buffer, int textSize, int width, int mergine) {
		...
		getLineView().setConstantMode(SHELL_BUFFER);
