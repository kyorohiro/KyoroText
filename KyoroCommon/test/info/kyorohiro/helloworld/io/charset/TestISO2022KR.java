...
public class TestISO2022KR extends TestCase {

	public void testFindESC() throws FileNotFoundException, IOException {
		File f = getTestFile("iso_2022_kr_crlf_001.txt");
		MarkableFileReader vm = new MarkableFileReader(f, 64);
		vm.seek(0);
		ISO2022KR iso2022kr = new ISO2022KR();
		iso2022kr.update(vm);
		{
			...
			iso2022kr.update(vm);
			assertArrayEquals(ISO2022.DESIGNATED(ISO2022.DESIGNATED_G1DM4, 'C'), iso2022kr.currentG1());
			assertArrayEquals(ISO2022.INVOKED_LS0, iso2022kr.currentG1Invoke());
			vm.read();vm.read();vm.read();vm.read();//ESC
			iso2022kr.update(vm);
			assertArrayEquals(ISO2022.INVOKED_LS1, iso2022kr.currentG1Invoke());
			vm.read();//ESC
			vm.read();vm.read();vm.read();vm.read();vm.read();vm.read();//äÿçëåÍÇ≈Ç±ÇÒÇ…ÇøÇÕ
			vm.read();vm.read();vm.read();vm.read();vm.read();vm.read();//äÿçëåÍÇ≈Ç±ÇÒÇ…ÇøÇÕ
			iso2022kr.update(vm);
			assertArrayEquals(ISO2022.INVOKED_LS0, iso2022kr.currentG1Invoke());
			assertArrayEquals(ISO2022.DESIGNATED(ISO2022.DESIGNATED_G1DM4, 'C'), iso2022kr.currentG1());
